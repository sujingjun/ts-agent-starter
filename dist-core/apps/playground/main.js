...
if (token.length < 32)
    throw new Error('API_AUTH_TOKEN 至少32字符');
...
async function body(req) { let value = ''; for await (const chunk of req) {
    value += String(chunk);
...
const server = createServer(async (req, res) => {
    ...
    try {
        ...
        if (req.url === '/run') {
            ...
            await runtime.runner.approve(tenant, input['runId'], input['callId'], input['allow']);
            res.end(JSON.stringify(await runtime.runner.execute(tenant, input['runId'])));
        }
        else {
            res.writeHead(404);
            res.end(JSON.stringify({ error: '不存在' }));
        }
    }
    catch (e) {
        res.writeHead(400);
        res.end(JSON.stringify({ error: e instanceof Error ? e.message : '请求失败' }));
    }
});
