...
export class PostgresRunStore {
    ...
    async create(state) { await this.db.query('INSERT INTO agent_runs(tenant_id,id,session_id,status,revision,body) VALUES($1,$2,$3,$4,$5,$6::jsonb)', [state.tenantId, state.id, state.sessionId, state.status, state.revision, JSON.stringify(state)]); }
    async get(tenant, id) { const r = await this.db.query('SELECT body FROM agent_runs WHERE tenant_id=$1 AND id=$2', [tenant, id]); return r.rows[0]?.['body']; }
    ...
    async list(tenant, limit = 50) { const r = await this.db.query('SELECT body FROM agent_runs WHERE tenant_id=$1 ORDER BY created_at DESC LIMIT $2', [tenant, Math.min(100, Math.max(1, limit))]); return r.rows.map(r => r['body']); }
}
/** 队列采用 PostgreSQL 租约。不依赖 Redis，不宣称跨外部 API 的恰好一次执行。 */
export class PostgresQueue {
    db;
    constructor(db) { this.db = db; }
    async enqueue(tenantId, runId) {
    ...
    async claim(owner, leaseSeconds = 60, tenantId) {
    ...
    async heartbeat(tenant, id, owner, leaseSeconds = 60) { const r = await this.db.query(`UPDATE agent_jobs SET lease_until=now()+($4*interval '1 second') WHERE tenant_id=$1 AND run_id=$2 AND owner=$3 AND status='active' AND lease_until>now() RETURNING run_id`, [tenant, id, owner, leaseSeconds]); return r.rows.length === 1; }
    async finish(tenant, id, owner, status) { await this.db.query('UPDATE agent_jobs SET status=$4,owner=NULL,lease_until=NULL WHERE tenant_id=$1 AND run_id=$2 AND owner=$3', [tenant, id, owner, status]); }
}
