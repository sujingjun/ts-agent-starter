...
export class NoteStore {
    ...
    async save(tenant, title, text) {
        ...
        const note = { id, title, text, createdAt: new Date().toISOString() };
    ...
    async list(tenant) {
        ...
        for (const name of await readdir(dir)) {
            if (/^[a-f0-9]{64}\.json$/.test(name))
                rows.push(JSON.parse(await readFile(join(dir, name), 'utf8')));
        }
        return rows.sort((a, b) => b.createdAt.localeCompare(a.createdAt)).slice(0, 100);
    }
}
export function noteTools(notes) {
    return [
        { name: 'save_note', description: '保存一条长期笔记，必须由用户批准这次调用。相同内容去重。',
            ...
            execute: async (args, ctx) => {
                ...
                return await notes.save(ctx.tenantId, args['title'], args['text']);
            } },
        { name: 'read_notes', description: '读取当前身份已保存的笔记；不能读取其他租户。',
            ...
            execute: async (_args, ctx) => ({ notes: await notes.list(ctx.tenantId) }) },
    ];
}
