...
export class SkillCatalog {
    ...
    add(skill) {
        if (this.map.has(skill.name))
            throw new AgentError('SKILL_DUPLICATE', '技能名称重复');
        this.map.set(skill.name, structuredClone(skill));
    }
    metadata() { return [...this.map.values()].map(({ name, description, sha256 }) => ({ name, description, sha256 })); }
    load(name) {
        const skill = this.map.get(name);
        if (!skill)
            throw new AgentError('SKILL_NOT_FOUND', '找不到技能');
        return structuredClone(skill);
    }
    async loadDirectory(root) {
