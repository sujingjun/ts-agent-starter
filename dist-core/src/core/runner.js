...
import { DefaultPolicy } from './tools.js';
...
export class AgentRunner {
    ...
    async create(input) {
        ...
        for (const key of ['maxSteps', 'maxToolCalls', 'maxRepeatedCalls'])
    ...
    async required(tenant, id) { const state = await this.store.get(tenant, id); invariant(state, 'NOT_FOUND', '运行不存在或不属于当前租户'); return state; }
    async record(state, type, data = {}, observer) {
    ...
    async approve(tenant, id, callId, allow) {
    ...
    async reconcile(tenant, id, callId, resolution) {
        const state = await this.required(tenant, id);
    ...
    async execute(tenant, id, options = {}) {
