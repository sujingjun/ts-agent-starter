...
export function chunkDocument(document, size = 600, overlap = 80) {
...
export class EvidenceIndex {
    ...
    add(document) {
        ...
        for (const c of chunkDocument(document))
            this.chunks.set(c.id, c);
    }
    get(tenantId, id) { const c = this.chunks.get(id); return c?.tenantId === tenantId ? structuredClone(c) : undefined; }
...
export function searchDocumentsTool(index) {
    return {
        ...
        async execute(input, context) { return { hits: index.search(context.tenantId, input['query']) }; },
    };
}
export function verifyCitations(answer, allowed) {
...
export function cosine(a, b) {
    if (a.length !== b.length || !a.length)
        throw new AgentError('VECTOR_DIMENSION', '向量维度不一致');
