...
export function validate(schema, value, path = '$', depth = 0) {
    const bad = (message) => { throw new AgentError('SCHEMA_VALIDATION', `${path}: ${message}`); };
    if (depth > 40)
        bad('数据嵌套过深');
...
export const s = {
    string: (maxLength = 4000) => ({ type: 'string', maxLength }),
    number: () => ({ type: 'number' }),
    object: (properties, required = Object.keys(properties)) => ({ type: 'object', properties, required, additionalProperties: false }),
};
