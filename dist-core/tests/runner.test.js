...
import { AgentRunner, MemoryRunStore, FileRunStore, ToolRegistry, ScriptedModel, calculatorTool, callTurn, finalTurn, s, DefaultPolicy } from '../src/core/index.js';
function setup(turns, tools = [calculatorTool()], store = new MemoryRunStore()) {
...
async function run(turns) { const r = setup(turns); const state = await r.runner.create({ tenantId: 'local', prompt: 'test' }); return r.runner.execute('local', state.id); }
function writeTool(execute) { return { name: 'publish', description: '模拟发布', effect: 'external', idempotent: false, timeoutMs: 100, inputSchema: s.object({ text: s.string() }), outputSchema: s.object({ sent: { type: 'boolean' } }), execute }; }
...
test('模型超时可终止等待', async () => { const model = { name: 'stall', complete: async () => new Promise(() => { }) }; const runner = new AgentRunner({ model, registry: new ToolRegistry(), store: new MemoryRunStore() }); const r = await runner.create({ tenantId: 'a', prompt: 'a', limits: { modelTimeoutMs: 10 } }); const state = await runner.execute('a', r.id); assert.equal(state.error?.code, 'TIMEOUT'); });
...
test('危险工具默认拒绝', async () => { const tool = { ...writeTool(async () => ({ sent: true })), effect: 'destructive' }; assert.equal(new DefaultPolicy().decide(tool, {}), 'deny'); });
