...
import type { Message, EvidenceDocument, JsonSchema } from '../src/core/index.js';
for (const [expression, expected] of [['2+3*4', 14], ['(2+3)*4', 20], ['-2*-3', 6], ['2--3', 5], ['0.5+.25', .75], ['10%3', 1], ['15+30+25', 70], ['+2', 2], ['(8/2)/(1+1)', 2]] as const)
...
test('Schema 严格对象', () => { const schema = s.object({ name: s.string(5), count: { type: 'integer', minimum: 1 } }); checkSchema(schema); assert.doesNotThrow(() => validate(schema, { name: '测试', count: 2 })); for (const bad of [{ name: 'a' }, { name: 'a', count: 0 }, { name: 'a', count: 1.2 }, { name: 'abcdef', count: 1 }, { name: 'a', count: 1, secret: true }])
    assert.throws(() => validate(schema, bad)); });
test('未知 Schema 不静默接受', () => assert.throws(() => checkSchema({ type: 'string', pattern: '.*' } as JsonSchema)));
...
test('Schema 数组边界', () => { const schema: JsonSchema = { type: 'array', items: s.number(), minItems: 1, maxItems: 2 }; assert.throws(() => validate(schema, [])); assert.throws(() => validate(schema, [1, 2, 3])); assert.throws(() => validate(schema, ['1'])); validate(schema, [1, 2]); });
test('参数规范化稳定', () => assert.equal(canonical({ a: 1, b: 2 }), canonical({ b: 2, a: 1 })));
const messages: Message[] = [{ role: 'system', content: '规则' }, { role: 'user', content: 'x'.repeat(3000) }, { role: 'assistant', content: 'old' }, { role: 'user', content: '新问题' }, { role: 'assistant', content: '', toolCalls: [{ id: 'c', name: 'calculator', arguments: { expression: '1+1' } }] }, { role: 'tool', toolCallId: 'c', content: '{"ok":true,"data":{"value":2}}' }];
...
const doc: EvidenceDocument = { id: 'doc', tenantId: 'a', title: '工具安全', text: '工具权限由服务端控制。审批并不等于无限授权。', source: 'local://doc', updatedAt: '2026-09-17' };
test('分块偏移对应原文', () => { for (const c of chunkDocument(doc, 10, 3))
