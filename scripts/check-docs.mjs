...
for(const f of new Set(docs)){const text=await readFile(f,'utf8');for(const match of text.matchAll(/\]\(([^)]+)\)/g)){const target=match[1].split('#')[0];if(!target||/^(https?:|mailto:|#)/.test(target)||target.includes('<'))continue;try{await access(resolve(dirname(f),decodeURIComponent(target)));}catch{errors.push(f+' → 失效相对链接：'+target);}}}
