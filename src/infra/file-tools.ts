...
import { AgentError, invariant } from '../core/errors.js';
...
export function fileTools(root: string): ToolDefinition[] {
    return [
        { name: 'list_files', description: '列出工作区指定目录，不跟随符号链接。', inputSchema: s.object({ path: s.string(500) }), outputSchema: s.object({ entries: { type: 'array', items: s.object({ name: s.string(500), kind: s.string(20) }) } }), effect: 'read', idempotent: true, timeoutMs: 2000,
            async execute(input) { const path = await safePath(root, input['path'] as string); const entries = await readdir(path, { withFileTypes: true }); return { entries: entries.slice(0, 100).map(e => ({ name: e.name, kind: e.isSymbolicLink() ? 'symlink' : e.isDirectory() ? 'directory' : 'file' })) }; } },
        { name: 'read_file', description: '读取工作区内普通 UTF-8 文件，返回 SHA-256 以便后续安全修改。', inputSchema: s.object({ path: s.string(500) }), outputSchema: s.object({ text: s.string(20000), sha256: s.string(64) }), effect: 'read', idempotent: true, timeoutMs: 2000,
            async execute(input) { const path = await safePath(root, input['path'] as string); const stat = await lstat(path); invariant(stat.isFile() && stat.size <= 20000, 'FILE_LIMIT', '仅支持不超过 20KB 的普通文件'); const text = await readFile(path, 'utf8'); return { text, sha256: createHash('sha256').update(text).digest('hex') }; } },
        { name: 'apply_patch', description: '精确替换文件中唯一一处文本，需要文件哈希匹配和人工审批；不创建新文件。', inputSchema: s.object({ path: s.string(500), expectedSha256: s.string(64), before: s.string(10000), after: s.string(10000) }), outputSchema: s.object({ sha256: s.string(64), changed: { type: 'boolean' } }), effect: 'write', idempotent: false, timeoutMs: 2000,
            async execute(input) {
                const path = await safePath(root, input['path'] as string);
        ...
        { name: 'review_diff', description: '对提供的 diff 做有限静态风险检查，返回匹配到的证据行，不冒充完整语义审查。', inputSchema: s.object({ diff: s.string(20000) }), outputSchema: s.object({ findings: { type: 'array', items: s.object({ line: s.number(), rule: s.string(), evidence: s.string() }) } }), effect: 'read', idempotent: true, timeoutMs: 1000,
            async execute(input) { const lines = (input['diff'] as string).split('\n'); const rules = [{ name: 'dynamic-evaluation', pattern: /\beval\s*\(|new\s+Function\s*\(/ }, { name: 'shell-interpolation', pattern: /\bexec\s*\(\s*`/ }, { name: 'unsafe-html', pattern: /innerHTML\s*=/ }]; return { findings: lines.flatMap((line, i) => line.startsWith('+') && !line.startsWith('+++') ? rules.filter(r => r.pattern.test(line)).map(r => ({ line: i + 1, rule: r.name, evidence: line })) : []) } as Json; } },
    ];
}
