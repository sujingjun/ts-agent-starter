...
export class ScriptedModel implements ModelPort {
    ...
    calls: ModelRequest[] = [];
    constructor(turns: ModelTurn[]) { this.turns = structuredClone(turns); }
    async complete(request: ModelRequest) { this.calls.push(request); const turn = this.turns.shift(); invariant(turn, 'SCRIPT_EXHAUSTED', '测试脚本没有更多响应'); return turn; }
}
export function finalTurn(content: string): ModelTurn { return { content, toolCalls: [], usage: usage0() }; }
export function callTurn(name: string, args: JsonObject, id = 'call-1'): ModelTurn { return { content: '', toolCalls: [{ id, name, arguments: args }], usage: usage0() }; }
...
export class DemoModel implements ModelPort {
    ...
    async complete(request: ModelRequest): Promise<ModelTurn> {
        ...
        if (last?.role === 'tool') {
            const output = JSON.parse(last.content) as {
                ok: boolean;
                data?: unknown;
                error?: {
                    message: string;
...
export class CompatibleChatModel implements ModelPort {
    ...
    async complete(request: ModelRequest): Promise<ModelTurn> {
        ...
        const raw = JSON.parse(await readBoundedText(response, this.options.maxResponseBytes ?? 2000000)) as WireResponse;
    ...
    private async parseStream(response: Response, request: ModelRequest): Promise<ModelTurn> {
        let content = '';
        let usage = usage0();
        let finished = false;
        ...
        for await (const data of sseData(response, this.options.maxResponseBytes ?? 2000000)) {
            ...
            const frame = JSON.parse(data) as WireResponse;
            if (frame.usage)
...
function readUsage(usage: WireResponse['usage']) { return { inputTokens: usage?.prompt_tokens ?? 0, outputTokens: usage?.completion_tokens ?? 0 }; }
...
export async function* sseData(response: Response, maxBytes = 2000000): AsyncGenerator<string> {
