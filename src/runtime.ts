...
import type { RunStore, EvidenceDocument } from './core/index.js';
...
export async function createRuntime(options: {
    ...
    const documents = JSON.parse(await readFile(resolve(root, 'data/knowledge.json'), 'utf8')) as EvidenceDocument[];
    ...
    for (const tool of noteTools(notes)) registry.register(tool);
